 ...
 * Date: 9/20/2016
 */
public class DoublyLinkedList<Anytype extends IDedObject>  {
	...
	public void swapAdjacent(int ID){
        ...
        if(previousLink != null){
        	...
        	secondLinkToSwap.previous = previousLink;
        	secondLinkToSwap.next = firstLinkToSwap;
        	firstLinkToSwap.previous = secondLinkToSwap;
        	firstLinkToSwap.next = endOfSwapChain;
        	endOfSwapChain.previous = firstLinkToSwap;
        }else{
        	secondLinkToSwap = firstLinkToSwap.next;
        	endOfSwapChain = firstLinkToSwap.next.next;
        	header = secondLinkToSwap;
        	secondLinkToSwap.previous = null;
        	secondLinkToSwap.next = firstLinkToSwap;
        	firstLinkToSwap.previous = secondLinkToSwap;
        	firstLinkToSwap.next = endOfSwapChain;
        	endOfSwapChain.previous = firstLinkToSwap;
	...
	public void printAllRecords(){
		...
		while(link != null && link.data != null){
			...
			if(link.previous != null)
				previous += link.previous.data.getID();
			else
				previous = "NULL";
				System.out.println("Previous Link ID = " + previous);
			System.out.println(link.data.toString());
			if(link.next.data != null)
				next += link.next.data.getID();
			else
				next = "NULL";
				System.out.println("Next Link ID = " + next);
			link = link.next;
		}
		System.out.println("\n*****************");
